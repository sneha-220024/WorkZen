...
const Payslips = () => {
    ...
    const handleGenerateAll = async () => {
        ...
        try {
            ...
            const response = await axios.post('http://localhost:5000/api/hr/payslip/generate-all', {
                month,
                year: parseInt(year)
            }, {
                headers: { Authorization: `Bearer ${token}` }
    ...
    const handleGenerateSingle = async (payrollId) => {
        try {
            ...
            const response = await axios.post(`http://localhost:5000/api/hr/payslip/generate/${payrollId}`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleDownload = async (payrollId, employeeName) => {
        try {
            ...
            const response = await axios({
                url: `http://localhost:5000/api/hr/payslip/download/${payrollId}`,
                method: 'GET',
                responseType: 'blob',
                headers: { Authorization: `Bearer ${token}` }
            });
