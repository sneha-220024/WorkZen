...
const LeaveManagement = () => {
    ...
    const fetchLeaves = async () => {
        try {
            ...
            if (!token) return;
            const res = await axios.get('http://localhost:5005/api/employee/leaves/history', {
    ...
    const handleApplyLeave = async (leaveData) => {
        try {
            ...
            const res = await axios.post('http://localhost:5005/api/employee/leaves/apply', payload, {
                headers: { Authorization: `Bearer ${token}` }
            });
